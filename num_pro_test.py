...
# Uploaded to AWS by John McClenon, Jr on 1/28/2023
...
orgcost_assum = pd.read_csv(
    'datafiles/orgcost_assum.csv', index_col='Organizational Costs')
equipur_assum = pd.read_csv(
    'datafiles/equipur_assum.csv', index_col='Equipment Purchases')
furpur_assum = pd.read_csv(
    'datafiles/furpur_assum.csv', index_col='Furniture & Fixtures')
offpur_assum = pd.read_csv(
    'datafiles/offpur_assum.csv', index_col='Office Equipment')
conlea_assum = pd.read_csv(
    'datafiles/conlea_assum.csv', index_col='Leasehold Improvements')
prepaid_assum = pd.read_csv(
    'datafiles/prepaid_assum.csv', index_col='PrePaid Expenses')
deppd_assum = pd.read_csv('datafiles/deppd_assum.csv',
                          index_col='Deposits Paid')
workcap_assum = pd.read_csv(
    'datafiles/workcap_assum.csv', index_col='Working Capital Assumptions')
sales_assum = pd.read_csv('datafiles/sales_assum.csv', index_col='Product')
operexp_assum = pd.read_csv(
    'datafiles/operexp_assum.csv', index_col='Operating Expenses')
wage_assum = pd.read_csv('datafiles/wage_assum.csv',
                         index_col='Salaries & Wages')
debt_assum = pd.read_csv('datafiles/debt_assum.csv', index_col='Lenders')
equity_assum = pd.read_csv(
    'datafiles/equity_assum.csv', index_col='Opening Equity')
cashprep_stmt = pd.read_csv(
    'datafiles/cashprep_stmt.csv', index_col='Cash Prep Statement')
cashflow_stmt = pd.read_csv(
    'datafiles/cashflow_stmt.csv', index_col='Cash Flow Statement')
balance_sheet = pd.read_csv(
    'datafiles/balance_sheet.csv', index_col='Balance Sheet')
income_stmt = pd.read_csv('datafiles/income_stmt.csv',
...
equipur_assum['Depreciation'] = (
    equipur_assum['Costs'] / (equipur_assum['Use_Life'] * 12)) * equipur_assum['Qty_Pur']
equipur_assum['Tot_Costs'] = equipur_assum['Costs'] * equipur_assum['Qty_Pur']
furpur_assum['Depreciation'] = (
    furpur_assum['Costs'] / (furpur_assum['Use_Life'] * 12)) * furpur_assum['Qty_Pur']
furpur_assum['Tot_Costs'] = furpur_assum['Costs'] * furpur_assum['Qty_Pur']
offpur_assum['Depreciation'] = (
    offpur_assum['Costs'] / (offpur_assum['Use_Life'] * 12)) * offpur_assum['Qty_Pur']
offpur_assum['Tot_Costs'] = offpur_assum['Costs'] * offpur_assum['Qty_Pur']
conlea_assum['Depreciation'] = (
    conlea_assum['Costs'] / (conlea_assum['Use_Life'] * 12))
orgcost_assum['Amortization'] = (
    orgcost_assum['Costs'] / (orgcost_assum['Use_Life'] * 12))
wage_assum['Tot_Mon_Wage'] = np.where(wage_assum['Pay_base'] == 'A',
                                      wage_assum['Sal_Amt'] / 12 *
                                      (1 + wage_assum['Fringe_Rate']),
                                      wage_assum['Sal_Amt'] * 40 * 4.33 * (1 + wage_assum['Fringe_Rate']))
debt_assum['Tot_Mon_DebtSer'] = - \
    npf.pmt(debt_assum.Int_Rate / 12, debt_assum.Term * 12, debt_assum.Loan_Amt)
...
tot_uses = tot_work_cap + start_inv + tot_prepaid + tot_equipur + tot_offpur + tot_furpur + tot_conlea + tot_orgcost \
    + tot_deppd
equity_proForma = np.where(
    implied_equity > 0, tot_uses - tot_debt, .2 * tot_debt)
cash_proForma = np.where(
    implied_equity > 0, tot_work_cap, equity_proForma - tot_debt)
...
for i in multipliers:
    ...
    if i < 1:
        ...
        pro_equip[f'ProForma'] = (
            equipur_assum['Costs'] * equipur_assum['Qty_Pur'])
        pro_furfix[f'ProForma'] = (
            furpur_assum['Costs'] * furpur_assum['Qty_Pur'])
        pro_offequ[f'ProForma'] = (
            offpur_assum['Costs'] * offpur_assum['Qty_Pur'])
...
pro_equipdepre.loc['Total Equip Depre'] = pro_equipdepre.sum(
...
pro_furfixdepre.loc['Total Fur & Fix Depre'] = pro_furfixdepre.sum(
    numeric_only=True, axis=0)

# This code creates the depreciation expense projections based on the fixed asset assumptions.
pro_offequdepre.index = offpur_assum.index
pro_offequdepre.index.name = 'Projected Off Equip Depre Expense'
pro_offequdepre.loc['Total Off Equip Depre'] = pro_offequdepre.sum(
    numeric_only=True, axis=0)
...
pro_consdepre.loc['Total Leasehold Depre'] = pro_consdepre.sum(
    numeric_only=True, axis=0)
...
pro_furfix.loc['Total Furniture & Fixtures'] = pro_furfix.sum(
    numeric_only=True, axis=0)
...
pro_offequ.loc['Total Office Equipment'] = pro_offequ.sum(
    numeric_only=True, axis=0)
...
pro_cons.loc['Total Leasehold Improvements'] = pro_cons.sum(
    numeric_only=True, axis=0)

# This code creates the Fixed Assets balance for the balance sheet.
fixed_assets = pro_equip.iloc[-1] + pro_furfix.iloc[-1] + \
    pro_offequ.iloc[-1]  # + pro_cons.iloc[-1]
...
mon_depre = pro_equipdepre.iloc[-1] + pro_furfixdepre.iloc[-1] + \
    pro_offequdepre.iloc[-1] + pro_consdepre.iloc[-1]
mon_depre.name = 'Monthly Depre'
pro_deprebalto = pro_deprebalto.append(mon_depre.transpose())
accum_depre = pro_equipdepre.iloc[-1] + pro_furfixdepre.iloc[-1] + \
    pro_offequdepre.iloc[-1] + pro_consdepre.iloc[-1]
...
for i in multipliers:
    if i == 0:
        ...
        pro_prepaidadd[f'per_{i}'] = np.where((i / 12 <= 1) & (i % 12 == 0),
                                              prepaid_assum['Costs'] *
                                              (1 + prepaid_assum['Inf_Rate']),
                                              np.where((i / 12 <= 2) & (i % 12 == 0),
                                                       prepaid_assum['Costs'] * (
                                                           (1 + prepaid_assum['Inf_Rate']) ** 2),
    ...
    if i == 0:
        ...
        pro_prepaidexp[f'per_{i}'] = pro_prepaidexp.iloc[:,
                                                         i - 12] * (1 + prepaid_assum['Inf_Rate'])
...
pro_othass.loc['Total Other Assets'] = pro_othass.sum(
    numeric_only=True, axis=0)
...
for i in multipliers:
    if i == 0:
        ...
        pro_intexp[f'per_{i}'] = -npf.ipmt(debt_assum.Int_Rate /
                                           12, i, debt_assum.Term * 12, debt_assum.Loan_Amt)
        pro_prin[f'per_{i}'] = -npf.ppmt(debt_assum.Int_Rate /
                                         12, i, debt_assum.Term * 12, debt_assum.Loan_Amt)
...
pro_intexp.loc['Total Interest Expense'] = pro_intexp.sum(
    numeric_only=True, axis=0)
...
pro_prin.loc['Total Principal Payments'] = pro_prin.sum(
    numeric_only=True, axis=0)
...
for i in multipliers:
    ...
    if i == 0:
        pro_curltdebt[f'ProForma'] = curltdebt_pro.iloc[:,
                                                        i + 1:i + 13].sum(axis=1)

    else:
        pro_curltdebt[f'per_{i}'] = curltdebt_pro.iloc[:,
                                                       i + 1:i + 13].sum(axis=1)
...
pro_orcostamor.loc['Total Org Costs Amort'] = pro_orcostamor.sum(
    numeric_only=True, axis=0)
...
pro_orcost.loc['Total Organizational Costs'] = pro_orcost.sum(
    numeric_only=True, axis=0)
...
for i in multipliers:
    if i < 1:
        ...
        pro_sales[f'per_{i}'] = sales_assum['Max_Mon_Sls'] * \
            (1 - np.exp(-i * sales_assum['Grow_Con']))
...
for i in multipliers:
    if i < 1:
        ...
        pro_operexp[f'per_{i}'] = pro_operexp.iloc[:,
                                                   i - 13] * (1 + operexp_assum['Inf_Rate'])
...
for i in multipliers:
    if i < 1:
        ...
        pro_othinc[f'per_{i}'] = deppd_assum['Amount'] * \
            (-deppd_assum['Rate_Earned'] / 12)
...
merbkfees = pro_sales.iloc[-1] * \
    workcap_assum.iloc[0, 3] * workcap_assum.iloc[0, 4]
...
ending_arbal = (pro_sales.iloc[-1] * workcap_assum.iloc[2, 3]
                ) / ((365 / workcap_assum.iloc[2, 5]) / 12)
...
ending_apbal = (pro_cogs.iloc[-1] * workcap_assum.iloc[4, 3]
                ) / ((365 / workcap_assum.iloc[4, 5]) / 12)
...
ending_wagespay = (pro_wages.iloc[-1] + pro_fringes.iloc[-1]
                   ) / ((365 / workcap_assum.iloc[5, 5]) / 12)
...
for i, col in cash_flow.iteritems():
    ...
    for j, row in cash_flow.iterrows():
        ...
        #            print(cash_flow.at[j, i])
            ...
            cash_flow.at[j, i] = -pro_cogs.at['Total COGS', i] - \
                pro_cashexp.at['Cash Expense', i]
        #            print(cash_flow.at[j, i])
        ...
        #            print(cash_flow.at[j, i])
            ...
            cash_gen = cash_flow.loc['CashSur(Def)', i] + \
                cash_flow.loc['TotExtFin', i]
        #    print(cash_gen)
...
totalassets = bal_sheet.loc['Net Fixed Assets': 'Total Other Assets'].sum(
) + bal_sheet.loc['Total Current Assets']
...
totalliabequity = bal_sheet.loc['Total Liabilities'] + \
    bal_sheet.loc['Total Equity']
